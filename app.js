...
function loadLocalPersistedData() {
  try {
    const savedLikes = localStorage.getItem('dangnyang_likes');
    if (savedLikes) {
      const likedMap = JSON.parse(savedLikes);
      appState.stories.forEach(s => {
        if (likedMap[s.id] !== undefined) {
          s.liked = likedMap[s.id].liked;
          s.likes = likedMap[s.id].likes;
        }
      });
    }
    const savedVols = localStorage.getItem('dangnyang_my_volunteers');
    if (savedVols) {
      appState.myVolunteers = JSON.parse(savedVols) || [];
    }
  } catch (e) {}
}

// Load Data from Local SEED_DATA
async function loadInitialData() {
  ...
  // 3. Fallback to embedded SEED_DATA
  appState.shelters = SEED_DATA.shelters;
  appState.stories = SEED_DATA.stories;
  appState.mbtiQuestions = SEED_DATA.mbtiQuestions;
  appState.mbtiResults = SEED_DATA.mbtiResults;
...
async function toggleStoryLike(storyId) {
  ...
  try {
    const saved = {};
    appState.stories.forEach(s => {
      saved[s.id] = { liked: s.liked, likes: s.likes };
    });
    localStorage.setItem('dangnyang_likes', JSON.stringify(saved));
  } catch (e) {}
...
async function submitVolunteerForm(e) {
  ...
  // Save to localStorage
  try {
    localStorage.setItem('dangnyang_my_volunteers', JSON.stringify(appState.myVolunteers));
  } catch (e) {}
